...
pub trait DrawModel<'model> {
    fn draw_mesh(&mut self, mesh: &'model Mesh, material: &'model Material);
    ...
    fn draw_model(&mut self, model: &'model Model);
    fn draw_model_instanced(&mut self, model: &'model Model, instances: Range<u32>);
}
